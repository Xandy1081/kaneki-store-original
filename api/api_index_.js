...
async function oauthStart(req,res,provider){
  ...
  // Keep OAuth state self-contained and cryptographically signed.
  // This avoids depending on a browser cookie surviving the external OAuth redirect.
  const statePayload = Buffer.from(JSON.stringify({
    nonce: crypto.randomBytes(24).toString('hex'),
    exp: Date.now() + 10 * 60 * 1000
  })).toString('base64url');
  const stateSig = crypto.createHmac('sha256', process.env.SESSION_SECRET)
    .update(statePayload).digest('base64url');
...
async function oauthCallback(req,res,provider){
  ...
  // Validate the signed state without requiring a cookie.
  const state = String(q.state || '');
  const dot = state.lastIndexOf('.');
  if (dot <= 0) return fail(res,400,'Estado OAuth inválido. Tente novamente.');
  const statePayload = state.slice(0, dot);
  ...
  try {
    ...
    const payload = JSON.parse(Buffer.from(statePayload,'base64url').toString('utf8'));
    if (!payload.exp || Number(payload.exp) < Date.now()) {
      return fail(res,400,'Estado OAuth expirado. Tente novamente.');
    }
    if (!payload.nonce || String(payload.nonce).length < 16) {
      return fail(res,400,'Estado OAuth inválido. Tente novamente.');
    }
  } catch {
    return fail(res,400,'Estado OAuth inválido. Tente novamente.');
...
const PIX_KEY = 'dina184513@gmail.com';
