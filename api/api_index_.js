...
async function db(){
  ...
  if(!schemaPromise) schemaPromise=(async()=>{
    ...
    const defaults={
      ...
      logoUrl:'images/kaneki-logo.png',
      backgroundImageUrl:'images/site-background-lilies.png',
      ...
      heroBannerUrl:'',
      heroBannerType:'none',
      heroBannerHeight:'320',
      heroBannerPosition:'center',
      heroBannerObjectFit:'cover',
      heroBannerAutoplay:'true',
      heroBannerLoop:'true',
      heroBannerMuted:'true',
      heroBannerOverlay:'0.25',
      heroBannerLink:'',
      customCss:'',
      pixFee:'1.49',
      whatsappNumber:'5591985748541',
      heroTitle:'𝕮𝖔𝖒𝖕𝖗𝖊 𝖋𝖆𝖈𝖎𝖑, 𝖗𝖆𝖕𝖎𝖉𝖔 𝖊 𝖘𝖊𝖌𝖚𝖗𝖔!',
      heroAccent:'',
      heroDescription:'Contas, gamepasses e frutas para Blox Fruits, com pagamento seguro via Pix e informações de entrega antes da compra.',
      heroButton:'Ver catálogo',
      heroBadge1:'✓ LOJA OFICIAL',
      heroBadge2:'⚡ ENTREGA AUTOMÁTICA',
      tutorialTitle:'Clique aqui',
      tutorialSubtitle:'Assista como comprar na KANEKI STORE',
      tutorialVideoUrl:'https://youtu.be/7aMOurgDB-o?is=XG9LU3hscFktyKOs',
      discordUrl:'https://discord.com/',
      supportUrl:'',
      whatsappUrl:'',
      footerText:'© 2026 KANEKI STORE. Todos os direitos reservados.',
      categories_json:JSON.stringify([
        {id:'produtos',title:'Frutas permanentes',text:'Aqui você vai encontrar as melhores frutas permanentes',button:'Ver produtos',image:'',color:'#e21b23',active:true},
        {id:'skins',title:'Skins',text:'Aqui você vai encontrar as melhores skins de frutas',button:'Ver produtos',image:'',color:'#e21b23',active:true},
        {id:'frutasfisicas',title:'Frutas físicas',text:'Aqui você vai encontrar as melhores frutas físicas',button:'Ver produtos',image:'',color:'#e21b23',active:true},
        {id:'gamepass',title:'Game Pass',text:'Aqui você vai encontrar os melhores Game Pass',button:'Ver produtos',image:'',color:'#e21b23',active:true},
        {id:'gachabox',title:'Gacha Box',text:'Aqui você vai encontrar os melhores itens da Gacha Box',button:'Ver produtos',image:'',color:'#e21b23',active:true}
      ]),
      primaryColor:'#e21b23',
      primaryDark:'#b91c1c',
    ...
    if(count.rows[0].n===0){
      for(const [id,p] of Object.entries(seed)){
        await pool.query(
          `INSERT INTO products(product_id,name,description,price,old_price,stock,image,catalog) VALUES($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT DO NOTHING`,
          [id,p.name,p.description||'',p.price,p.old,String(p.stock),p.image||'',p.catalog||'produtos']
        );
      }
    }
...
function publicUser(r){
  return {id:r.id,email:r.email,name:r.name,phone:r.phone||'',role:r.role};
}

async function log(action,email=''){
  await pool.query('INSERT INTO logs(action,email) VALUES($1,$2)',[action,email||null]).catch(()=>{});
}
...
async function catalog(res){
  ...
  await db();
  const q=await pool.query('SELECT product_id,name,description,price,old_price AS old,stock,image,catalog FROM products WHERE active=true ORDER BY product_id');
  const rows=q.rows.map(p=>({...p,image:(String(p.image||'').startsWith('images/')?'':p.image)}));
  return ok(res,{products:rows});
}

async function me(req,res){
  const t=auth(req);
  if(!t) return ok(res,{user:null,cart:{}});
  await db();
  const u=await pool.query('SELECT * FROM users WHERE id=$1',[t.id]);
  if(!u.rowCount) return ok(res,{user:null,cart:{}});
  const c=await pool.query('SELECT cart FROM carts WHERE user_id=$1',[t.id]);
...
async function loginRegister(req,res,isRegister){
  ...
  if(isRegister){
    ...
    if(owner && process.env.OWNER_PASSWORD && password!==process.env.OWNER_PASSWORD){
      return fail(res,403,'Para criar a conta do dono, use a senha configurada em OWNER_PASSWORD.');
    }

    const id=crypto.randomUUID();
    const role=owner?'owner':'customer';

    const r=await pool.query(
      'INSERT INTO users(id,email,name,phone,password_hash,role,provider) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING *',
      [id,email,String(b.name||email.split('@')[0]).slice(0,100),String(b.phone||'').slice(0,40),hashPassword(password),role,'password']
    );

    u=r.rows[0];
    await log('register',email);
  }else{
    if(!existing.rowCount || !checkPassword(password,existing.rows[0].password_hash)){
  ...
  const c=await pool.query('SELECT cart FROM carts WHERE user_id=$1',[u.id]);

  return ok(res,{
    user:publicUser(u),
    token,
    cart:c.rowCount?c.rows[0].cart:{}
  });
}

async function cart(req,res){
  const t=auth(req);
  if(!t)return fail(res,401,'Faça login para salvar o carrinho.');
  await db();
  const b=await body(req);

  await pool.query(
    `INSERT INTO carts(user_id,cart,updated_at)
     VALUES($1,$2,NOW())
     ON CONFLICT(user_id) DO UPDATE SET cart=EXCLUDED.cart,updated_at=NOW()`,
    [t.id,JSON.stringify(b.cart||{})]
  );
...
async function orders(req,res){
  const t=auth(req);
  if(!t)return fail(res,401,'Faça login para registrar o pedido.');
  await db();

  const b=await body(req);
  const items=Array.isArray(b.items)?b.items:[];

  if(!items.length)return fail(res,400,'Carrinho vazio.');

  const orderKey=String(b.orderKey||crypto.randomUUID());

  const r=await pool.query(
    `INSERT INTO orders(user_id,order_key,items,total,name,email,phone)
    ...
    [
      ...
      String(b.email||t.email||''),
      String(b.phone||'')
    ]
  );

  await log('order',t.email);

  return ok(res,{orderId:r.rows[0].id,orderKey});
}

async function myOrders(req,res){
  const t=auth(req);
  if(!t)return fail(res,401,'Faça login primeiro.');
  await db();

  const r=await pool.query(
    'SELECT id,order_key,items,total,name,email,phone,status,created_at AS "createdAt" FROM orders WHERE user_id=$1 ORDER BY created_at DESC',
    [t.id]
  );

  return ok(res,{orders:r.rows});
}
...
async function tickets(req,res){
  ...
  if(req.method==='POST'){
    ...
    if(!o.rowCount)return fail(res,404,'Compra não encontrada.');

    const r=await pool.query(
      'INSERT INTO tickets(user_id,order_id,subject) VALUES($1,$2,$3) RETURNING id',
      [t.id,b.orderId,String(b.subject||'Atendimento')]
    );

    await pool.query(
      'INSERT INTO ticket_messages(ticket_id,sender_role,name,message) VALUES($1,$2,$3,$4)',
      [r.rows[0].id,'customer',t.email,String(b.message||'')]
    );
  }
  ...
  const q=isOwner
    ? await pool.query(`SELECT t.*,o.order_key,o.name AS customer_name,o.email
                        FROM tickets t
  ...
  for(const row of q.rows){
    const m=await pool.query(
      'SELECT sender_role,name,message,created_at AS "createdAt" FROM ticket_messages WHERE ticket_id=$1 ORDER BY created_at',
      [row.id]
    );

    out.push({...row,messages:m.rows});
  }
...
async function ticketReply(req,res){
  ...
  const q=['owner','admin'].includes(t.role)
    ? await pool.query('SELECT * FROM tickets WHERE id=$1',[b.ticketId])
    : await pool.query('SELECT * FROM tickets WHERE id=$1 AND user_id=$2',[b.ticketId,t.id]);

  if(!q.rowCount)return fail(res,404,'Ticket não encontrado.');

  const role=t.role==='owner'?'owner':(t.role==='admin'?'admin':'customer');

  await pool.query(
    'INSERT INTO ticket_messages(ticket_id,sender_role,name,message) VALUES($1,$2,$3,$4)',
    [b.ticketId,role,t.name||t.email,String(b.message||'')]
  );

  await pool.query(
    'UPDATE tickets SET status=COALESCE($2,status),updated_at=NOW() WHERE id=$1',
    [b.ticketId,b.status||null]
  );
...
async function chatGeneral(req,res){
  ...
  if(req.method==='POST'){
    ...
    await pool.query(
      'INSERT INTO chat_messages(user_id,name,message) VALUES($1,$2,$3)',
      [t.id,String(t.name||t.email).slice(0,100),message]
    );
  ...
  const q=await pool.query(
    `SELECT c.id,c.name,c.message,c.created_at AS "createdAt",u.role
...
async function adminTickets(req,res){
  ...
  if(!t || !['owner','admin'].includes(t.role)){
    return fail(res,403,'Acesso restrito aos administradores.');
  }

  await db();

  const q=await pool.query(
    `SELECT t.*,o.order_key,o.name AS customer_name,o.email
     FROM tickets t
     LEFT JOIN orders o ON o.id=t.order_id
     ORDER BY t.updated_at DESC`
  );
...
async function adminRoles(req,res){
  ...
  if(!t || !isOwnerEmail(t.email) || t.role!=='owner' || t.provider!=='google'){
    return fail(res,403,'Somente o dono pode gerenciar administradores.');
  }
  ...
  if(!userId || !['grant','revoke'].includes(action)){
    return fail(res,400,'Dados inválidos.');
  }

  const q=await pool.query('SELECT * FROM users WHERE id=$1',[userId]);

  if(!q.rowCount)return fail(res,404,'Usuário não encontrado.');
  if(isOwnerEmail(q.rows[0].email)){
    return fail(res,400,'O dono não pode perder o próprio acesso.');
  }
...
async function reviews(req,res){
  ...
  if(req.method==='GET'){
    const q=await pool.query(
      `SELECT r.id,r.rating,r.message,r.product_text AS "productText",
              r.created_at AS "createdAt",
              COALESCE(u.name,'Cliente verificado') AS name,
  ...
  if(!t)return fail(res,401,'Faça login para avaliar uma compra.');

  const b=await body(req);
  const rating=Math.max(1,Math.min(5,Number(b.rating||0)));
  const message=String(b.message||'').trim();

  if(!rating||!message)return fail(res,400,'Informe a nota e a avaliação.');
  if(message.length>300)return fail(res,400,'A avaliação pode ter até 300 caracteres.');
  ...
  if(orderId){
    ...
    if(!q.rowCount)return fail(res,404,'Pedido não encontrado.');
    order=q.rows[0];
  }else{
    const q=await pool.query(
    ...
    if(!q.rowCount)return fail(res,400,'Faça uma compra antes de avaliar.');
    order=q.rows[0];
  }

  const productText=(Array.isArray(order.items)?order.items:[])
    .map(i=>`${i.name||'Produto'}${i.qty>1?' × '+i.qty:''}`)
    .join(', ')
    .slice(0,220);

  const q=await pool.query(
    `INSERT INTO reviews(user_id,order_id,rating,message,product_text)
     VALUES($1,$2,$3,$4,$5)
     ...
     RETURNING id`,
    [t.id,order.id,rating,message,productText]
  );

  await log('review',t.email);

  return ok(res,{ok:true,reviewId:q.rows[0].id});
}
...
async function robloxLookup(req,res){
  ...
  try{
    ...
    try{
      ...
      return fail(res,502,'O Roblox retornou uma resposta inválida.');
    }
    ...
    if(!user)return fail(res,404,'Usuário do Roblox não encontrado.');
    ...
    return fail(res,502,'Não foi possível consultar o Roblox agora.');
  }
}
...
async function siteSettings(res){
  ...
  const q=await pool.query('SELECT key,value FROM site_settings');
...
async function adminSettings(req,res){
  ...
  if(!t || !isOwnerEmail(t.email) || t.role!=='owner' || t.provider!=='google'){
    return fail(res,403,'Acesso restrito: entre com Google usando dina184513@gmail.com.');
  }
...
async function adminSummary(req,res){
  ...
  const [u,o,tic,p,l]=await Promise.all([
  ...
  const sq=await pool.query('SELECT key,value FROM site_settings');
...
async function adminProduct(req,res,del=false){
  ...
  if(del){
    await pool.query(
      'UPDATE products SET active=false,updated_at=NOW() WHERE product_id=$1',
      [b.productId]
    );
  ...
  await pool.query(
    `INSERT INTO products(product_id,name,description,price,old_price,stock,image,catalog,active,updated_at)
...
async function oauthStart(req,res,provider){
  ...
  if(!cfg.id){
    return fail(
      res,
      500,
      `${provider.toUpperCase()}_CLIENT_ID não configurado na Vercel.`
    );
  }

  if(!process.env.SESSION_SECRET){
    return fail(res,500,'SESSION_SECRET não configurado na Vercel.');
  }

  // State independente do cookie/sessão, assinado com SESSION_SECRET.
  const nonce=crypto.randomBytes(32).toString('hex');
  const statePayload=`${provider}.${nonce}`;
  ...
  u.searchParams.set(
    'redirect_uri',
    `${baseUrl(req)}/api/oauth/${provider}/callback`
  );
...
async function oauthCallback(req,res,provider){
  ...
  // Valida o state sem depender de cookie.
  const state=String(q.state||'');
  const parts=state.split('.');

  if(parts.length!==3 || parts[0]!==provider){
    return fail(res,400,'Estado OAuth inválido. Tente novamente.');
  }

  const stateProvider=parts[0];
  const nonce=parts[1];
  ...
  if(!code){
    return fail(
      res,
      400,
      q.error_description||'Código OAuth ausente.'
    );
  }
  ...
  if(provider==='google'){
    if(!process.env.GOOGLE_CLIENT_SECRET){
      return fail(
        res,
        500,
        'GOOGLE_CLIENT_SECRET não configurado na Vercel.'
      );
    }
    ...
    if(!tr.ok || !tok.access_token){
      console.error('Google token error:',tok);
      return fail(res,502,'Google recusou o login OAuth.');
    }
    ...
    if(!process.env.DISCORD_CLIENT_SECRET){
      return fail(
        res,
        500,
        'DISCORD_CLIENT_SECRET não configurado na Vercel.'
      );
    }
    ...
    if(!tr.ok || !tok.access_token){
      return fail(res,502,'Discord recusou o login OAuth.');
    }
    ...
    if(!pr.ok){
      return fail(
        res,
        502,
        'Não foi possível obter os dados da conta Discord.'
      );
    }
  }
  ...
  if(!email){
    return fail(res,400,'O provedor não retornou um e-mail.');
  }
  ...
  if(!u.rowCount){
    ...
    u=await pool.query(
      `INSERT INTO users(id,email,name,phone,role,provider,avatar_url)
    ...
    if(isOwnerEmail(email) && u.rows[0].role!=='owner'){
      u=await pool.query(
        "UPDATE users SET role='owner' WHERE id=$1 RETURNING *",
        [u.rows[0].id]
      );
    }
  }
  ...
  return res.writeHead(302,{
    Location:`${baseUrl(req)}/?login=success`
  }).end();
}
...
module.exports = async (req,res) => {
  try{
    ...
    if(route==='login' && req.method==='POST'){
      return loginRegister(req,res,false);
    }

    if(route==='register' && req.method==='POST'){
      return loginRegister(req,res,true);
    }
    ...
    if(route==='ticket-reply' && req.method==='POST'){
      return ticketReply(req,res);
    }

    if(route==='chat'){
      return chatGeneral(req,res);
    }

    if(route==='admin/tickets'){
      return adminTickets(req,res);
    }

    if(route==='admin/roles' && req.method==='POST'){
      return adminRoles(req,res);
    }

    if(route==='reviews'){
      return reviews(req,res);
    }

    if(route==='roblox'){
      return robloxLookup(req,res);
    }

    if(route==='settings'){
      return siteSettings(res);
    }
    ...
    if(route==='admin/product' && req.method==='POST'){
      return adminProduct(req,res,false);
    }

    if(route==='admin/product/delete' && req.method==='POST'){
      return adminProduct(req,res,true);
    }

    const m=route.match(/^oauth\/(google|discord)(?:\/callback)?$/);

    if(m){
      const provider=m[1];
      return route.endsWith('/callback')
        ? oauthCallback(req,res,provider)
        : oauthStart(req,res,provider);
    }
