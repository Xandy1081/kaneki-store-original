...
const ASAAS_SANDBOX_URL = process.env.ASAAS_API_BASE_URL || 'https://api-sandbox.asaas.com/v3';

async function getActiveAsaasPixKey(){
  const configured=String(process.env.ASAAS_PIX_KEY||'').trim();
  if(configured) return configured;
  const data=await asaasRequest('/pix/addressKeys?status=ACTIVE&limit=100&offset=0',{method:'GET',headers:{'content-type':'application/json'}});
  const keys=Array.isArray(data?.data)?data.data:[];
  const active=keys.find(k=>String(k?.status||'').toUpperCase()==='ACTIVE' && String(k?.key||'').trim());
  if(!active) throw new Error('Nenhuma chave Pix ATIVA foi encontrada na conta Asaas. Cadastre/ative a chave aleatória (EVP) no Sandbox.');
...
async function asaasRequest(path, options={}){
  ...
  const response=await fetch(`${ASAAS_SANDBOX_URL}${path}`,{
    ...options,
    headers:{
...
async function createPix(req,res){
  try{
    ...
    let qr;
    try{
      qr=await asaasRequest('/pix/qrCodes/static',{
        method:'POST',
        body:JSON.stringify({
          addressKey:await getActiveAsaasPixKey(),
          description:`Kaneki Store ${orderKey}`.slice(0,100),
          value:amount,
          format:'ALL',
      ...
      return fail(res,e.status||502,e.message||'Não foi possível gerar o PIX no Asaas.');
    }
    if(!qr?.id || !qr?.payload){
      await pool.query('UPDATE orders SET status=$1 WHERE id=$2',['payment_error',orderId]).catch(()=>{});
      return fail(res,502,'O Asaas não retornou um QR Code válido.');
    }
    ...
    await pool.query(
      ...
      [orderId,orderKey,qr.id,amount,nick,expiresAt]
    );
    let qrImage='';
    ...
    return ok(res,{ok:true,orderId,orderKey,qrCodeId:qr.id,amount,fee,subtotal,payload:qr.payload,qrImage,expiresAt});
  }catch(e){
  ...
  try{
    ...
    if(['PAYMENT_RECEIVED','PAYMENT_CONFIRMED'].includes(event)){
      ...
      if(qrId){
        ...
        await pool.query(
          `UPDATE pix_orders SET status='RECEIVED',asaas_payment_id=$1,event_id=COALESCE(event_id,$2),paid_at=NOW() WHERE asaas_payment_id=$1`,
          [paymentId,eventId]
        );
      }
      const q=qrId
        ? await pool.query('SELECT order_id FROM pix_orders WHERE qr_code_id=$1 LIMIT 1',[qrId])
        : await pool.query('SELECT order_id FROM pix_orders WHERE asaas_payment_id=$1 LIMIT 1',[paymentId]);
