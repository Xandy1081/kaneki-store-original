...
const ASAAS_API_BASE_URL = String(process.env.ASAAS_API_BASE_URL || 'https://api-sandbox.asaas.com/v3').replace(/\/$/,'');

async function asaasRequest(path, options={}){
  const key=process.env.ASAAS_API_KEY;
  if(!key) throw new Error('ASAAS_API_KEY não configurado na Vercel.');
  ...
  if(!response.ok){
    const detail=data?.errors?.map?.(x=>x.description||x.code).filter(Boolean).join('; ') || data?.error || data?.message || `Asaas HTTP ${response.status}`;
    const err=new Error(detail); err.status=response.status; err.data=data; throw err;
  }
  return data;
}

function normalizeCpfCnpj(value){ return String(value||'').replace(/\D/g,''); }
function validCpfCnpjLength(value){ const v=normalizeCpfCnpj(value); return v.length===11 || v.length===14; }

async function createAsaasCustomer({name,cpfCnpj,email,mobilePhone,externalReference}){
  const cpf=normalizeCpfCnpj(cpfCnpj);
  if(!validCpfCnpjLength(cpf)) throw new Error('Informe um CPF (11 dígitos) ou CNPJ (14 dígitos) válido para gerar o PIX.');
  const customer=await asaasRequest('/customers',{
    method:'POST',
    body:JSON.stringify({
      name:String(name||'Cliente KANEKI STORE').trim().slice(0,100),
      cpfCnpj:cpf,
      email:String(email||'').trim().slice(0,100) || undefined,
      mobilePhone:String(mobilePhone||'').replace(/\D/g,'').slice(0,20) || undefined,
      externalReference:String(externalReference||'').slice(0,100),
      notificationDisabled:true
    })
  });
  if(!customer?.id) throw new Error('O Asaas não retornou o cliente da cobrança.');
  return customer;
}

async function createAsaasPixPayment({customerId,amount,orderKey}){
  ...
  const payment=await asaasRequest('/payments',{
    ...
    body:JSON.stringify({
      ...
      value:amount,
      dueDate,
      description:`Kaneki Store ${orderKey}`.slice(0,500),
      externalReference:orderKey
    })
  });
  if(!payment?.id) throw new Error('O Asaas não retornou o ID da cobrança PIX.');
  return payment;
}

async function getAsaasPixQrCode(paymentId){
  const qr=await asaasRequest(`/payments/${encodeURIComponent(paymentId)}/pixQrCode`,{method:'GET'});
  if(!qr?.payload) throw new Error('O Asaas não retornou o PIX Copia e Cola da cobrança.');
  return qr;
}
...
async function createPix(req,res){
  try{
    ...
    const cpfCnpj=normalizeCpfCnpj(b.cpfCnpj||b.cpf||'');
    if(!validCpfCnpjLength(cpfCnpj)) return fail(res,400,'Informe um CPF (11 dígitos) ou CNPJ (14 dígitos) para gerar o PIX.');
    ...
    const t=auth(req);
    const customerName=String(b.name||t?.name||'Cliente KANEKI STORE').trim().slice(0,100) || 'Cliente KANEKI STORE';
    const customerEmail=String(b.email||t?.email||'').trim();
    ...
    const order=await pool.query(
      ...
      [t?.id||null,orderKey,JSON.stringify(items),amount,customerName,customerEmail,customerPhone]
    );
    const orderId=order.rows[0].id;
    let payment, qr, customer;
    try{
      customer=await createAsaasCustomer({name:customerName,cpfCnpj,email:customerEmail,mobilePhone:customerPhone,externalReference:orderKey});
      payment=await createAsaasPixPayment({customerId:customer.id,amount,orderKey});
      qr=await getAsaasPixQrCode(payment.id);
    }catch(e){
      await pool.query('UPDATE orders SET status=$1 WHERE id=$2',['payment_error',orderId]).catch(()=>{});
      return fail(res,e.status||502,e.message||'Não foi possível gerar o PIX dinâmico no Asaas.');
    }
    const expiresAt=qr.expirationDate || new Date(Date.now()+1799*1000).toISOString();
    // Mantemos qr_code_id preenchido por compatibilidade com o schema atual;
    // no PIX dinâmico o identificador principal é o payment.id.
    await pool.query(
      `INSERT INTO pix_orders(order_id,order_key,qr_code_id,amount,status,nick,expires_at,asaas_payment_id) VALUES($1,$2,$3,$4,'PENDING',$5,$6,$7)`,
      [orderId,orderKey,payment.id,amount,nick,expiresAt,payment.id]
    );
    let qrImage='';
    if(qr.encodedImage) qrImage=String(qr.encodedImage).startsWith('data:')?String(qr.encodedImage):`data:image/png;base64,${qr.encodedImage}`;
    return ok(res,{ok:true,orderId,orderKey,qrCodeId:payment.id,asaasPaymentId:payment.id,customerId:customer.id,amount,fee,subtotal,payload:qr.payload,qrImage,expiresAt,expirationDate:qr.expirationDate||null});
  }catch(e){
...
async function pixStatus(req,res){
  try{
    ...
    const q=await pool.query(`SELECT p.order_key,p.status,p.amount,p.expires_at AS "expiresAt",p.paid_at AS "paidAt",p.asaas_payment_id AS "asaasPaymentId",o.status AS "orderStatus" FROM pix_orders p LEFT JOIN orders o ON o.id=p.order_id WHERE p.order_key=$1 LIMIT 1`,[key]);
    if(!q.rowCount)return fail(res,404,'Pedido PIX não encontrado.');
    return ok(res,{ok:true,...q.rows[0]});
...
async function asaasWebhook(req,res){
  try{
    ...
    if(!expected || !received || received!==expected) return fail(res,401,'Webhook não autorizado.');
    await db();
    const b=await body(req);
    const eventId=String(b.id||'').trim();
    const event=String(b.event||'').trim();
    const payment=b.payment||{};
    if(!eventId) return fail(res,400,'Evento sem id.');
    const inserted=await pool.query(
      `INSERT INTO pix_orders_event_guard(event_id) VALUES($1) ON CONFLICT DO NOTHING RETURNING event_id`,
      [eventId]
    ).catch(async()=>{
      return {rowCount:0,rows:[]};
    });
    if(inserted.rowCount===0){
      // If the guard table is unavailable or this is a duplicate, still return 200.
      return ok(res,{received:true,duplicate:true});
    }
    if(['PAYMENT_RECEIVED','PAYMENT_CONFIRMED'].includes(event)){
      const qrId=String(payment.pixQrCodeId||'').trim();
      const paymentId=String(payment.id||'').trim();
      if(paymentId){
        await pool.query(
          `UPDATE pix_orders SET status='RECEIVED',asaas_payment_id=$1,event_id=COALESCE(event_id,$2),paid_at=NOW() WHERE (asaas_payment_id=$1 OR qr_code_id=$1) AND status<>'RECEIVED'`,
          [paymentId,eventId]
        );
      }
      if(qrId){
        await pool.query(
          `UPDATE pix_orders SET status='RECEIVED',asaas_payment_id=COALESCE(NULLIF($1,''),asaas_payment_id),event_id=COALESCE(event_id,$2),paid_at=NOW() WHERE qr_code_id=$3 AND status<>'RECEIVED'`,
          [paymentId,eventId,qrId]
        );
      }
      const q=paymentId
        ? await pool.query('SELECT order_id FROM pix_orders WHERE asaas_payment_id=$1 OR qr_code_id=$1 LIMIT 1',[paymentId])
        : (qrId
          ? await pool.query('SELECT order_id FROM pix_orders WHERE qr_code_id=$1 LIMIT 1',[qrId])
          : {rowCount:0,rows:[]});
      if(q.rowCount && q.rows[0].order_id){
        await pool.query(`UPDATE orders SET status='paid' WHERE id=$1`,[q.rows[0].order_id]);
      }
    }else if(['PAYMENT_REFUNDED','PAYMENT_OVERDUE','PAYMENT_DELETED','PAYMENT_RESTORED'].includes(event)){
      const qrId=String(payment.pixQrCodeId||'').trim();
      const paymentId=String(payment.id||'').trim();
      const statusMap={PAYMENT_REFUNDED:'refunded',PAYMENT_OVERDUE:'expired',PAYMENT_DELETED:'cancelled',PAYMENT_RESTORED:'pending'};
      if(paymentId){
        await pool.query(`UPDATE pix_orders SET status=$1 WHERE (asaas_payment_id=$2 OR qr_code_id=$2) AND status<>'RECEIVED'`,[statusMap[event],paymentId]);
      }else if(qrId){
        await pool.query(`UPDATE pix_orders SET status=$1 WHERE qr_code_id=$2 AND status<>'RECEIVED'`,[statusMap[event],qrId]);
      }
    }
    return ok(res,{received:true});
  }catch(e){
    console.error('asaasWebhook',e);
    return fail(res,500,'Erro ao processar webhook.');
  }
}
...
module.exports = async (req,res) => {
 try{
  ...
  if(route==='asaas-webhook' && req.method==='POST') return asaasWebhook(req,res);
