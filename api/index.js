...
async function getAsaasCustomerId(){
  const id=String(process.env.ASAAS_CUSTOMER_ID||'').trim();
  if(!id) throw new Error('ASAAS_CUSTOMER_ID não configurado na Vercel. Crie/seleciona um cliente no Asaas e informe o ID dele.');
  return id;
}
...
async function createPix(req,res){
  try{
    ...
    let payment, qr, customerId;
    try{
      customerId=await getAsaasCustomerId();
      payment=await createAsaasPixPayment({customerId,amount,orderKey});
    ...
    return ok(res,{ok:true,orderId,orderKey,qrCodeId:payment.id,asaasPaymentId:payment.id,customerId,amount,fee,subtotal,payload:qr.payload,qrImage,expiresAt,expirationDate:qr.expirationDate||null});
  }catch(e){
